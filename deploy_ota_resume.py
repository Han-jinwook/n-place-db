...
def main():
    print("3. Renaming folders to spoof 1.1.72 for the old broken updater (Resume)")
    temp_dir = "temp_dist"
    if os.path.exists(temp_dir):
        # We don't want to crash if we can't delete it, so we'll just try to clear it
        try:
    ...
    for build_type in ["PRO", "TRIAL"]:
        ...
        new_folder = os.path.join(temp_dir, f"NPlace-DB-{build_type}-v1.1.72")
        
        if os.path.exists(new_folder):
            shutil.rmtree(new_folder, ignore_errors=True)
            
        print(f"Copying {old_folder} to {new_folder}...")
