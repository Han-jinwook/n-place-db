...
if st.session_state['active_page'] == 'Shop Search':
    ...
    tab_collect, tab_email, tab_insta = st.tabs(["🔍 수집", "📧 이메일", "💬 인스타"])
    
    with tab_collect:
        with st.container(border=True):
            st.subheader(" 🔎 플레이스 수집 가이드")
            st.markdown("""
            NPlace-DB 수집 엔진은 네이버 스마트플레이스 데이터를 안전하게 실시간으로 수집합니다.
            
            #### **수집 기본 방법**
            1. **지역 선택**: 좌측 사이드바에서 원하는 **시/도**와 **구/군**을 선택하세요. 여러 개 선택도 가능합니다.
            2. **키워드 입력**: '피부샵', '네일아트' 등 타겟 업종을 콤마(,)로 구분하여 한 개 이상 입력하세요.
            3. **수집 시작**: **[▶ 수집 시작]** 버튼을 누르면 백그라운드 엔진이 가동되며 실시간 로그창에 진행 상황이 표시됩니다.
            
            #### **안전하고 효율적인 수집 팁**
            * **IP 차단 방지**: 너무 짧은 수집 지연 시간은 IP 차단을 유발할 수 있으니 기본 설정값을 유지하는 것을 권장합니다.
            * **중도 정지**: 작업 중 언제든 **[■ 수집 중지]** 버튼을 눌러 멈출 수 있으며, 그때까지 수집된 데이터는 모두 안전하게 저장됩니다.
            """)
    ...
    with tab_email:
        with st.container(border=True):
            ...
            **[중요] 로그인 실패(535) 해결 방법**  
            ...
            5. 생성된 **16자리 영문 앱 암호**를 복사하여 프로그램의 '이메일 비밀번호' 칸에 입력하세요.
            
            ---
            #### **기타 계정 SMTP 설정 안내**
            *   **Gmail**: 구글 계정 보안 설정에서 '2단계 인증'을 켜고 **'앱 비밀번호'**를 생성해야 합니다.
    ...
    with tab_insta:
        with st.container(border=True):
            ...
            st.markdown("""
            인스타그램 마케팅 엔진은 **진짜 크롬 브라우저**를 띄워 사람과 동일한 방식으로 전송하는 고성능 엔진입니다.
            ...
            2. **처음 사용하는 계정**일 경우, 인스타 보안 정책에 따라 2단계 인증이나 **로그인 본인 확인**이 필요할 수 있습니다. 
            3. 팝업으로 뜨는 브라우저 창에서 직접 로그인을 한 번만 완료해 주세요. (인증 세션이 암호화되어 저장되므로 다음부터는 자동 로그인됩니다.)
            
            #### **3단계: [실전 노하우] 스팸 폴더를 피하고 성공율 300% 올리는 법**
            *   **발송 계정 최적화 (Warm-up)**:
                *   가입 후 거의 사용하지 않은 깡통 계정은 인스타 필터링 봇의 최우선 타겟입니다.
                *   **평소 다른 사람들과 디엠을 나누고, 릴스를 보며 좋아요를 누르던 활동적인 실사용 계정**일수록 차단 필터를 쉽게 통과합니다.
                *   계정에 **프로필 사진을 등록**하고, 소개란을 채우며, 일반 사진 게시물을 2~3개 이상 유지하세요.
            *   **메시지 개인화 (치환 태그 활용)**:
                *   동일한 메시지만 복붙해서 대량 발송하면 즉시 섀도우 밴(Shadow-ban)에 걸립니다. 
                *   본문에 `{상호명}` 태그를 섞어서 발송하면 각 업체마다 제목이 바뀌므로 스팸 차단 확률이 극적으로 줄어듭니다.
            *   **"메시지 요청(Requests)"의 심리 활용**:
                *   서로 팔로우가 아닌 상태에서 DM을 보내면 일반 편지함이 아닌 **[메시지 요청]** 탭으로 들어갑니다.
                *   자영업자/원장님들은 인스타 DM을 통해 고객 예약 문의가 들어오기 때문에, 모르는 계정의 요청도 **"혹시 고객인가?"** 하고 거의 100% 열어봅니다.
                *   따라서 첫 문장에 무작정 "홍보/광고" 단어를 넣기보다는 **"안녕하세요 {상호명} 원장님! 중요한 제안/문의가 있어 연락드렸습니다."** 처럼 호기심을 유발하는 접근이 유리합니다.
            *   **네이버 톡톡과 병행 강력 추천**:
                *   인스타 DM이 막혀있거나 계정이 없는 업체는 **[이메일]**이나 직접 전화를 활용하세요!
            """)

    # [NEW] Guide page end Customer Support link
    st.divider()
    st.subheader("💡 프로그램 이용 중 해결되지 않은 문제가 있나요?")
    st.markdown("3Monster 통합 고객센터를 통해 오류 로그와 함께 문의해 주시면 전문가가 신속히 답변 드리겠습니다.")
    st.markdown("""
