...
### 4.8. 제품별 패키징 독립 실행 파일명 필수 적용 수칙 (Unique Exe Naming Rule)
- **규칙**: 동일 엔진/소스에서 생성되는 멀티 제품군(`CafeCrawler`, `EventStats`, `AutoComment`, `Trial` 등)을 개별 패키지 ZIP으로 스테이징할 때, 압축 내부 실행 파일명을 공통 범용명(`CafeScraper.exe`)으로 동일하게 패키징해서는 안 되며, 각 제품 전용 명칭(`CafeCrawler.exe`, `EventStats.exe`, `AutoComment.exe`, `CafeMonster-Trial.exe`)으로 각각 변경하여 결합해야 합니다.
- **사유**: 사용자가 여러 패키지 상품을 동시에 구매하거나 한 폴더/다운로드 디렉터리에서 압축을 해제할 때, 실행 파일명이 같아 덮어쓰기 오류가 나거나 어떤 프로그램인지 구별하지 못하는 사용자 경험(UX) 오류를 완전 차단합니다.

---
*Updated on 2026-08-24 by Antigravity*
