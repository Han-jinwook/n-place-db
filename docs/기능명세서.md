...
- **이력 관리 및 초기화**: `trial_logs` 테이블에 UPSERT(merge-duplicates) 방식으로 누적 기록되며, 어드민 대시보드의 `프로그램별 체험판 현황`에서 개별 사용자의 HWD(하드웨어 ID) 기록을 초기화/삭제하여 재체험 기회를 유연하게 부여할 수 있음.
...
2. **Versioned Naming Rule (Zip)**: 배포 스크립트(`deploy_ota.py`)는 각 빌드 결과를 `Map_DB-Pro-v{Version}.zip`과 `Map_DB-Trial-v{Version}.zip` 이라는 **버전 번호가 기재된 파일명**으로 압축하여 GitHub Releases 에 업로드한다. 다만 압축 내부의 최상위 폴더명과 실행 파일명(`.exe`)은 버전을 제거하여 단축 아이콘 경로 호환성 및 업데이트 안정성을 보장한다.
3. **Dashboard Integration**: 3Monster 웹 대시보드(어드민)에서는 Supabase `app_versions` 테이블에 등록된 최신 버전 전용 다운로드 링크를 참조하여 유저에게 직접 버전이 포함된 파일을 안전하게 다운로드 받을 수 있도록 연동되어 있다.

### ♻️ OTA Auto-Updater (`updater.py`)
- 앱 기동 시 `updater.py`는 Supabase의 `app_versions` 테이블을 폴링하여 다운로드 링크를 획득한다.
- 획득한 URL이 `-Pro`를 가리키고 있으나 현재 실행 중인 클라이언트가 `TRIAL` 빌드라면, `updater.py`는 다운로드 URL 내부의 `-Pro` 문자열을 `-Trial`로 런타임 치환(`re.sub(r'-Pro', '-Trial', download_url)`)하여 자신에게 맞는 버전을 정확하게 다운로드 받는다. (그 반대도 동일)
- 다운로드한 압축 파일 내부의 최상위 폴더명을 동적으로 인식하여, 동일한 이름의 `.exe` 실행 파일을 찾아 자동으로 덮어쓰기 및 재실행을 완수한다.
...
## 6. [UI 표준] 대시보드 통계 초기화 (UX/UI 가이드라인)

앞으로 개발되는 모든 프로그램(앱)의 대시보드에는 공통적으로 **[통계 초기화 후 작업하기]** 기능이 포함되어야 한다.

### 🧹 통계 초기화 버튼
  - UI 레이아웃의 `데이터 관리` 패널 하단에 `🧹 통계 초기화 후 작업하기` 버튼 형태로 제공.
  - 클릭 시 다음과 같은 동작을 수행.
    1. 로컬 저장소의 SQLite DB 및 `progress.json` 파일 등 로컬 상태 기록 안전하게 삭제 (혹은 아카이빙)
    2. 화면에 표시되는 렌더링 통계값(DB Count, Last Saved 등)을 리셋하고 앱 리프레시(`st.rerun()`)

### 🚨 [중요 보안] 체험판 무한 루프 어뷰징 방지
  - 사용자가 "통계 초기화"를 통해 로컬 데이터를 삭제하더라도, 체험판 횟수 한도를 무한으로 리셋하여 사용하는 꼼수(Exploit)를 원천 차단해야 한다.
  - 이를 위해 대시보드 로직(`app.py`)에서 **크롤러/작업 엔진 프로세스가 시작(Popen)되기 직전에 반드시** `AuthManager.get_and_sync_trial_used_count()` 등 서버(Supabase)와 통신하는 함수를 호출하여 **진짜 누적 사용량 기준점을 강제로 동기화(`_trial_baseline`)**해야 한다. 
  - 동기화 후, `총 제한 횟수 - 서버 누적 횟수`로 남은 횟수를 철저하게 계산하여 엔진에 `limit` 인자로 전달해야 한다.

## 7. 빌드 파이프라인 보안 (Build & Distribution)
  - 파이썬 스크립트 특성상, 배포된 `.exe` 실행 파일은 쉽게 디컴파일(Decompile)될 위험이 존재한다.
  - 따라서 상용 배포용 빌드 스크립트(`build_exe.py` 등)는 PyInstaller 구동 전, 반드시 **PyArmor** 등의 난독화/암호화 도구를 사용하여 핵심 비즈니스 로직 소스 코드들을 1차적으로 난독화해야 한다.
