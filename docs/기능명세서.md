...
2. **Static Naming Rule (Zip & Folder)**: 배포 스크립트(`deploy_ota.py`)는 각 빌드 결과를 `NPlace-DB-Pro.zip`과 `NPlace-DB-Trial.zip` 이라는 **버전 번호가 제외된 고정된 파일명**으로 압축하여 GitHub Releases 에 업로드한다. 또한 압축 내부의 최상위 폴더명과 실행 파일명(`.exe`) 역시 버전을 제거하여 단축 아이콘 경로 호환성을 보장한다.
...
- 획득한 URL이 `Pro.zip`을 가리키고 있으나 현재 실행 중인 클라이언트가 `TRIAL` 빌드라면, `updater.py`는 다운로드 URL 내부의 `Pro.zip` 문자열을 `Trial.zip`으로 런타임 치환(`download_url.replace("Pro.zip", "Trial.zip")`)하여 자신에게 맞는 버전을 정확하게 다운로드 받는다.
...
- **목적**: 사용자가 여러 차례 수집 작업을 반복할 때, 이전에 추출해둔 엑셀/데이터 원본 파일은 그대로 유지하면서 **현재 대시보드 화면에 누적된 집계 수치(총 업체수, 마지막 수집 시간 등)만 깔끔하게 0으로 리셋**하여 새로운 작업의 혼동을 방지하기 위함.
- **구현 방식**: 
  - UI 레이아웃의 `데이터 관리` 패널 하단에 `🧹 통계 초기화 후 작업하기` 버튼 형태로 제공.
  - 버튼 클릭 시 로컬 상태 DB(`NPlace-DB.sqlite` 등)와 `progress.json` 파일만 삭제/비우기 처리함.
  - 백그라운드 엔진 구동 중(PID 존재 시)에는 예기치 않은 오류를 막기 위해 해당 버튼을 **비활성화(Disabled)** 처리해야 함.
