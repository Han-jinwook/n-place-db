...
class MainApp(ctk.CTk):
    def __init__(self):
        ...
        self.title(f"[{config.BRAND_NAME_KR}] 플레이스 DB Basic V1.0")
    ...
    def setup_ui(self):
        ...
        self.label_header = ctk.CTkLabel(self.header_frame, text=f"{config.BRAND_NAME_KR} 플레이스 DB Basic", 
    ...
    def start_crawling(self):
        ...
        thread = threading.Thread(target=self.run_crawler_process, args=(area, count, keyword, exclude), daemon=True)
        thread.start()

    def run_crawler_process(self, area, count, shop_type, exclude=""):
        try:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "-m":
            ...
            # e.g., "step1_refined_crawler.py" or an absolute path
            import os
            import runpy
            arg1 = sys.argv[1]
            ...
            if script_path:
                ...
                try:
                    ...
                    l.error(f"Error running {base_name}: {e}", exc_info=True)
                    sys.exit(1)
                sys.exit(0)
            else:
                sys.exit(1)
            ...
            # caused by Streamlit's internal subprocess or watchdog calls.
            sys.exit(0)
    ...
    try:
        ...
        if run_auth_flow(is_pro=False):
            logger.info("Authentication successful. Launching main dashboard launcher.")
            
            # Explicitly call our new launcher for Basic Version
            logger.info("Calling main_launcher_basic.main() ...")
            import main_launcher_basic
            try:
                main_launcher_basic.main() 
            except Exception as e:
                logger.error(f"Error in main_launcher_basic: {e}", exc_info=True)
                sys.exit(1)
        else:
